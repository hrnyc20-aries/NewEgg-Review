            ...
            <ul className="filterOptions">
              ...
              <li>
                <label className="filterCheckbox">
                  <input
                    type="checkbox"
                    onChange={(e) => this.props.handleFilter('verified')}
                  />
                  ...
                  <div className="ratingView" key={'key' + eggCount}>
                    <div className="ratingViewLine">
                      ...
                      <div className="ratingChart">
                        <div
                          className="ratingChart_current"
                          style={{ width: `${this.getPercent(eggCount)}%` }}
                        />
                        <div className="ratingNumber">
                          {this.getNumberOfRatings(eggCount)}
                        </div>
                      </div>
                      <div className="ratingPercent">
                        {' '}
                        {Math.round(this.getPercent(eggCount)) || 0}%
                      </div>
                    </div>
                  </div>
                );
        ...
        <div className="toolBar">
          <div className="sortTool">
            ...
            <div className="reviewCount">
              Reviews{' '}
              <strong>
                {this.props.reviews.length > 0 ? 1 : 0}-
                {this.props.reviews.length}
              </strong>{' '}
              of <strong>{this.props.reviews.length}</strong>
            </div>
          </div>
        </div>
      </div>
